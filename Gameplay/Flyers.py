...
class Crow(Flyer):
	...
	def __init__(self, x, y, direction, gold = "none", reverse = False):
		...
		self.directory = str(Path("Assets/Cro"))
		...
		self.audio = pygame.mixer.Sound(str(Path("Assets/Audio/Misc/bird_fly.wav")))
	...
	def _load_images(self):
		...
		fly_images = SpriteSheet( str( Path(self.directory)/self.filename1 ) ).load_grid(start_rect1, 3, 2, -1)

		idle_images = SpriteSheet(str(Path(f"{self.directory}/{self.filename2}"))).load_grid(start_rect2, 4, 2, -1)
...
class Angels(Flyer):
	...
	def __init__(self, x, y):
		...
		self.filename = str(Path("Assets/images/sheets/ending_animations.png"))
		...
		self.audio = pygame.mixer.Sound(str(Path("Assets/Audio/Misc/plink.wav")))
